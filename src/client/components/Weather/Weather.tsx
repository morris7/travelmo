...
const Weather: React.FunctionComponent<IWeatherProps> = ({ className, text }) => {
  const classes = classNames(ROOT_CLASSNAME, className);

  return (
    <section
      className={classes}
      dangerouslySetInnerHTML={{ __html: weatherSvg + '<h2>Weather</h2>' + text }}
    />
  );
}
